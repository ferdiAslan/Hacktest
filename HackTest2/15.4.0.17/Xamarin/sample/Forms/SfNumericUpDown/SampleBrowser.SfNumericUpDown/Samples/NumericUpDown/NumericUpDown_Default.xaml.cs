...
namespace SampleBrowser.SfNumericUpDown
{
    public partial class NumericUpDown_Default : SampleView
    {
        ...
        void Minimum_ValueChanged(object sender, Xamarin.Forms.ValueChangedEventArgs e)
        {
            int minimum = (int)e.NewValue;
            if (minimum != (int)AppleCount.Minimum)
            {
                if (minimum < AppleCount.Maximum)
                {
                    AppleCount.Minimum = PomegranateCount.Minimum = OrangeCount.Minimum = BananaCount.Minimum = minimum;
                    minimumValueDisplayLabel.Text = minimum.ToString();
                }
                else if (minimum >= AppleCount.Maximum)
                {
                    AppleCount.Minimum = PomegranateCount.Minimum = OrangeCount.Minimum = BananaCount.Minimum = AppleCount.Maximum - 1;
                    minimumSlider.Value = AppleCount.Maximum - 1;
                    minimumValueDisplayLabel.Text = AppleCount.Minimum.ToString();
                }
            }
        }

        void Maximum_ValueChanged(object sender, Xamarin.Forms.ValueChangedEventArgs e)
        {
            int maximum = (int)e.NewValue;

            if (maximum > (int)AppleCount.Minimum)
            {
                AppleCount.Maximum = PomegranateCount.Maximum = OrangeCount.Maximum = BananaCount.Maximum = (double)maximum;
                maximumValueDisplyLabel.Text = maximum.ToString();
            }
            else if (maximum <= AppleCount.Minimum)
            {
                AppleCount.Maximum = PomegranateCount.Maximum = OrangeCount.Maximum = BananaCount.Maximum = AppleCount.Minimum + 1;
                maximumSlider.Value = AppleCount.Minimum + 1;
                maximumValueDisplyLabel.Text = BananaCount.Maximum.ToString();
            }
        }
