...
namespace SampleBrowser
{
		...
		public class ScheduleDelegate : SFScheduleDelegate
		{
			...
			public override void didSelectDate(SFSchedule schedule, NSDate selectedDate, NSArray appointments)
			{
				...
				if (appointments != null && appointments.Count > 0)
				{
					...
					{
						if (schedule.Appointments.GetItem<ScheduleAppointment>(i) == appointments.GetItem<ScheduleAppointment>(0))
						{
							indexOfAppointment = int.Parse(i.ToString());
							break;
						}
					}
					selectedAppointment = (appointments.GetItem<ScheduleAppointment>(0));
					...
					label_subject.Text = "Subject";
					label_location.Text = "Location";
					String _sDate = DateTime.Parse((selectedDate.ToString())).ToString();
					picker_startDate.SetDate(selectedDate, true);
					button_startDate.SetTitle(_sDate, UIControlState.Normal);
					String _eDate = DateTime.Parse((selectedDate.AddSeconds(3600).ToString())).ToString();
					picker_endDate.SetDate(selectedDate.AddSeconds(3600), true);
					button_endDate.SetTitle(_eDate, UIControlState.Normal);

				}
			}

			public override void didVisibleDates(SFSchedule schedule, NSArray visibleDates)
			{
				...
				NSDate date = visibleDates.GetItem<NSDate>((nuint)CurrentVisibleMonth);
				NSDateFormatter dateFormat = new NSDateFormatter();
				dateFormat.DateFormat = "MMMM YYYY";
	...
	public class ScheduleDataSource : SFScheduleDataSource
	{
		NSMutableArray Data;
		...
		NSMutableArray CreateAppointments()
		{
			...
			appointment.StartTime = startDate;
			appointment.EndTime = endDate;
			appointment.Subject = new NSString(@"B'Day");
			appointment.AppointmentBackground = UIColor.Blue;
			...
			appointment1.AppointmentBackground = UIColor.Orange;
			NSMutableArray appCollection = new NSMutableArray();
			appCollection.Add(appointment);
		...
		#region implemented abstract members of SFScheduleDataSource

		public override NSArray GetAppointmentsForDate(SFSchedule schedule, NSDate date)
		{
			NSCalendar calendar = NSCalendar.CurrentCalendar;
			NSDateComponents components = calendar.Components(NSCalendarUnit.Year | NSCalendarUnit.Month | NSCalendarUnit.Day, (NSDate)date);
			components.Hour = 23;
			components.Minute = 59;
			components.Second = 59;
			NSDate rangeEndDateWithTime = calendar.DateFromComponents(components);
			NSPredicate predicate = NSPredicate.FromFormat(@"(startTime <= %@) AND (startTime >= %@)", rangeEndDateWithTime, (NSDate)date);
			return Data.Filter(predicate);
		}

		#endregion

		public override NSArray GetAppointmentsInDateRange(SFSchedule schedule, NSDate rangeStartDate, NSDate rangeEndDate)
		{
			NSCalendar calendar = NSCalendar.CurrentCalendar;
			NSDateComponents components = calendar.Components(NSCalendarUnit.Year | NSCalendarUnit.Month | NSCalendarUnit.Day, rangeEndDate);
			components.Hour = 23;
			components.Minute = 59;
			components.Second = 59;
			NSDate rangeEndDateWithTime = calendar.DateFromComponents(components);
			NSPredicate predicate = NSPredicate.FromFormat(@"(startTime <= %@) AND (startTime >= %@)", rangeEndDateWithTime, rangeStartDate);
			return Data.Filter(predicate);
		}
